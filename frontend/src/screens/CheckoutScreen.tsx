...
export default function CheckoutScreen() {
  ...
  const payFood = async () => {
    ...
    try {
       ...
       * Pago DEMO de comida.
       *
       * No necesita reservationId ni
       * llamada al backend de reservas.
       */
        ...
        [
          {
            ...
            onPress: () => {
              ...
              /*
               * Regresamos a Cine creando una
               * nueva pantalla para que el carrito
               * vuelva vacío.
               */

              navigation.replace('Cine');
  ...
  const payTickets = async () => {
    ...
    if (!reservationId) {
      ...
      Alert.alert(
        'Error',
        'No encontramos la reserva de esta compra.',
      );
    ...
    if (!token) {
      ...
      Alert.alert(
        ...
        'No pudimos validar tu sesión. Regresa e intenta nuevamente.',
      );
    ...
    try {
      ...
      if (!response?.reservation) {
        ...
        throw new Error(
          'El servidor no devolvió la reserva pagada.',
        );
      ...
      const tickets =
        response.reservation.tickets ?? [];


      if (tickets.length === 0) {

        throw new Error(
          'El pago fue procesado, pero no se generó ningún ticket.',
        );
      ...
      /*
       * Mostramos el primer ticket.
       *
       * Mis Tickets tendrá los demás.
       */
      ...
      const qrPayload =
        `ticketsafe:v1:${ticket.id}:${ticket.qrCodeHash}`;
  ...
  const cancelCurrentReservation =
    async () => {

      /*
       * La comida todavía no tiene
       * reservationId.
       */
      ...
      >
        ...
        <View style={styles.paymentCard}>
          ...
          <Text style={styles.demoText}>
            ...
            Modo demostración: puedes usar los
            datos precargados.
        ...
        <AppButton
          label={
            isFoodPurchase
              ? `Pagar comida · $${paymentTotal.toFixed(2)}`
              : `Pagar ahora · $${paymentTotal.toFixed(2)}`
          }
          onPress={() => void pay()}
          loading={processing}
          disabled={processing}
